...
class WebSeleniumComponent(DirectiveProvider, CommandProvider):
    ...
    @command(
        ...
        ),
        {
            ...
            "topics_of_interest": JSONSchema(
                ...
                description=("A list of topics about which you want to extract information " "from the page."),
                required=False,
            ),
            "question": JSONSchema(
                type=JSONSchema.Type.STRING,
                description=("A question you want to answer using the content of the webpage."),
                required=False,
            ),
        ...
        *,
        ...
        try:
            ...
            if not text:
                ...
                if (
                    output_tokens := self.llm_provider.count_tokens(text, self.model_info.name)
                ) > MAX_RAW_CONTENT_LENGTH:
                    oversize_factor = round(output_tokens / MAX_RAW_CONTENT_LENGTH, 1)
                    raise TooMuchOutputError(
                        f"Page content is {oversize_factor}x the allowed length " "for `get_raw_content=true`"
                    )
                return text + (f"\n\nLinks: {links}" if links else "")
            else:
                text = await self.summarize_webpage(text, question or None, topics_of_interest)
            ...
            if (chromium_driver_path := Path("/usr/bin/chromedriver")).exists():
                ...
                try:
                    ...
                    if "'NoneType' object has no attribute 'split'" in str(e):
                        ...
                        logger.critical("Connecting to browser failed:" " is Chrome or Chromium installed?")
                    raise
    ...
    def _sideload_chrome_extensions(self, options: ChromeOptions, dl_folder: Path) -> None:
